#Przepraszam za tą zaległość, ale ostatnio miałam problemy z prądem i przez to przeoczyłam to zadanie. Pozdrawiam :)
...
class Testy(unittest.TestCase):
    def test_dostepnosc_ksiazki(self):
        library = Library(["Krew Elfow", "Czas Pogardy"])
        self.assertEqual(library.availableBooks, ["Krew Elfow", "Czas Pogardy"])
        self.assertTrue("Czas Pogardy" in library.availableBooks)
        self.assertFalse("Wieza Jaskolki" in library.availableBooks)
        
    def test_posiadanie_ksiazki(self):
        user = Customer()
        user.book = "Pani Jeziora"
        user.haveBook = True
        self.assertEqual(user.book, "Pani Jeziora")
        self.assertTrue(user.haveBook)
